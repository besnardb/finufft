...
#include <cufinufft/hip/spreadinterp.hpp>
...
__device__ __forceinline__ subprob_block_info<T, ndim> compute_subprob_block_info(
  ...
  std::array<int, 3> binsizes{p.opts.gpu_binsizex, p.opts.gpu_binsizey,
                                    p.opts.gpu_binsizez};
...
template<typename T, int Ndim> struct bin_layout {
  ...
  std::array<int, 3> nbins;
  int nbins_tot;
  std::array<T, 3> inv_binsizes;
};

template<typename T, int Ndim>
inline bin_layout<T, Ndim> compute_bin_layout(
    const cufinufft_opts &opts, const std::array<CUFINUFFT_BIGINT, 3> &nf123) {
  std::array<int, 3> binsizes{opts.gpu_binsizex, opts.gpu_binsizey,
                                    opts.gpu_binsizez};
  auto nbins      = get_nbins<Ndim>(nf123, binsizes);
  const int total = nbins_total(nbins);
  std::array<T, 3> inv{T(1) / binsizes[0], T(1) / binsizes[1], T(1) / binsizes[2]};
